...
impl Invaders {
    pub fn new() -> Self {
        ...
        for x in 0..NUM_COLS {
            for y in 0..NUM_ROWS {
                if x > 1 && x < NUM_COLS - 2 && y > 0 && y < 9 && x % 2 == 0 {
    ...
    pub fn update(&mut self, delta: Duration) -> bool {
        ...
        if self.move_timer.ready {
            ...
            if self.direction == 1 {
                ...
                let mix_x = self.army.iter().map(|invader| invader.x).min().unwrap_or(0);
                if mix_x == 0 {
                    self.direction = 1;
                    is_downward = true
                }
            ...
            if is_downward {
                ...
                for invader in self.army.iter_mut() {
                    invader.x = (invader.x as i8 + self.direction) as usize;
                }
            }
            return true;
        }
